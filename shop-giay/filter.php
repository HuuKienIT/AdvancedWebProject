...
if ($_REQUEST['search'] == "" && $_REQUEST['nhanhieu'] == "" && $_REQUEST['danhmuc'] == "") {
    ...
    if (count($brands) == 0 && count($sizes) == 0) {
        ...
        $statement = "select * from tbl_product join tbl_pro_soluong on tbl_product.id_pro=tbl_pro_soluong.id_pro where soluong>0 and id_size IN ($placeholders)  and giamoi >= $giamin and giamoi <= $giamax  $ghi  limit $sophantu offset $batdau ";
        $para = $sizes;
        ...
        $statement = "select * from tbl_product join tbl_pro_soluong on tbl_product.id_pro=tbl_pro_soluong.id_pro where soluong>0 and id_nh IN ($placeholders1) AND id_size IN ($placeholders2)  and giamoi >= $giamin and giamoi <= $giamax $ghi limit $sophantu offset $batdau ";
...
?>
    ...
    if (count($brands) == 0 && count($sizes) == 0) {
        ...
        $placeholders = implode(',', array_fill(0, count($sizes), '?'));
        $statement = "select *  from tbl_product join tbl_pro_soluong on tbl_product.id_pro=tbl_pro_soluong.id_pro where soluong>0 and id_size IN ($placeholders)  and giamoi >= $giamin and giamoi <= $giamax ";
        $para = $sizes;
        ...
        $placeholders1 = implode(',', array_fill(0, count($brands), '?'));
        $placeholders2 = implode(',', array_fill(0, count($sizes), '?'));
        $statement = "select * from tbl_product join tbl_pro_soluong on tbl_product.id_pro=tbl_pro_soluong.id_pro where soluong>0 and id_nh IN ($placeholders1) AND id_size IN ($placeholders2)  and giamoi >= $giamin and giamoi <= $giamax ";
