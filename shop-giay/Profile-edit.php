...
if (isset($_POST['form1'])) {
	...
	if ($valid == 1) {
		...
		$db = new Helper();
		$stmt = "UPDATE tbl_users SET ten_user=?, email=?, sodth=? ,diachi =? WHERE id_user=?";
		$para = [$_POST['ten_user'], $_POST['email'], $_POST['sodth'], $_POST['diachi'], $_SESSION['user1']['id_user']];
...
if(isset($_POST["huydh"])){
	...
	$stmt = "select * from tbl_chitiet_px where id_px=?";
	$para["id_px"];
	$result = $db->fetchAll($stmt,$para);
	foreach ($result as $spnh) {
		$db = new Helper();
