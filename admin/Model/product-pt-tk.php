...
$id_dm = $_GET['id_dm'];
$conn = new Helper();
$stmt = "SELECT * FROM tbl_product join tbl_danhmuc on tbl_product.id_dm = tbl_danhmuc.id_dm join tbl_nhanhieu on tbl_product.id_nh = tbl_nhanhieu.id_nh where tbl_product.daxoa <>1 and (id_pro regexp ? or ten_pro regexp ?) AND tbl_product.id_nh regexp ? AND tbl_product.id_dm regexp ?";
...
<?php
$conn = new Helper();
$stmt = "SELECT * FROM tbl_product join tbl_danhmuc on tbl_product.id_dm = tbl_danhmuc.id_dm join tbl_nhanhieu on tbl_product.id_nh = tbl_nhanhieu.id_nh where tbl_product.daxoa <>1 and (id_pro regexp ? or ten_pro regexp ?) AND tbl_product.id_nh regexp ? AND tbl_product.id_dm regexp ? limit $sodong offset $min ";
$para = [$search, $search, $id_nh, $id_dm];
...
?>
    <tr class="dong">
        ...
        <td class="text-center"><?php echo $row['total_view']; ?></td>
        ...
        <td>
            <a id="sua" href="index.php?page=product-edit&id=<?php echo $row['id_pro']; ?>" class="btn btn-primary btn-xs">Edit</a>
            <a id="xoa" href="#" class="btn btn-danger btn-xs" data-href="index.php?page=product-delete&id=<?php echo $row['id_pro']; ?>" data-toggle="modal" data-target="#confirm-delete">Delete</a>
        </td>
